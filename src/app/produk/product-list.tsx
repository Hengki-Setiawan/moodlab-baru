...
export function ProductList() {
  ...
  const productsQuery = useMemoFirebase(() => {
    ...
    const productsCollection = collection(firestore, 'digitalProducts') as CollectionReference<DigitalProduct>;
    return firestoreQuery(productsCollection);
  }, [firestore]);
  ...
  useEffect(() => {
    ...
    const clientKey = "SB-Mid-client-35fgBhK8ianqJP3d";
  ...
  const handleBuy = async (product: DigitalProduct) => {
    ...
    try {
        ...
        if (response.ok) {
            // Snap API memberikan token yang dapat digunakan dengan snap.js
            if (transaction.token) {
                // Pastikan snap sudah dimuat
                if (!(window as any).snap) {
                    throw new Error('Snap.js belum dimuat dengan benar. Silakan muat ulang halaman.');
                }
                ...
                (window as any).snap.pay(transaction.token, {
                    ...
                    onPending: function(result: any){
                        // Untuk pembayaran yang memerlukan tindakan lebih lanjut
                ...
                // Beberapa metode pembayaran mungkin memberikan redirect_url
